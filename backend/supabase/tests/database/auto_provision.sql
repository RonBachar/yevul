...
-- ====================================================================
-- קבוצה 6, בדיקת מצב סופי, לא בדיקת ההגנה מפני יצירה כפולה עצמה.
--
-- הערה חשובה: אין כאן הדמיה אמיתית של הפעלה חוזרת של הטריגר. אי אפשר
-- להכניס שוב אותו user_id ל-auth.users (מפתח ראשי), ואי אפשר לקרוא
-- ל-handle_new_user() ישירות מחוץ להקשר טריגר. לכן ה-guard clause
-- שבתוך handle_new_user() (ה-if exists בתחילת הפונקציה, ראה המיגרציה
-- 20260820130000) נשאר קוד לא נבדק בפועל, וגם, כפי שההערה במיגרציה
-- עצמה מודה, הוא לא יכול להיות true עבור אף הכנסה אמיתית של משתמש
-- חדש, זו רשת ביטחון תיאורטית בלבד. מה שהבדיקות למטה כן מוודאות הוא
-- שהמצב הסופי אחרי ההרשמות בקבוצות הקודמות הוא בדיוק משק אחד לכל
-- משתמש, לא יותר.
-- ====================================================================
