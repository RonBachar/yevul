...
const strings: Record<string, string> = {
  ...
  // ליד היומן ולא ליד ההגדרות: המחירון הוא מה שממלא את העלות ביומן
  // הריסוס, ושם החקלאי מחפש אותו.
  'web.nav.pricelist': 'מחירון',
  ...
  // הוצאות החלקות, בשורת הפירוק מתחת למספר הגדול. "החלקות" ולא
  // "הוצאות" סתם, כי מאז 2026-09-25 יש במסך שני מספרי הוצאות ורק אחד
  // מהם נכנס לחישוב.
  'home.profit.plotExpenses': 'הוצאות החלקות',
  // בלוק הוצאות המשק, מתחת לכרטיס. פיצ'ר בתשלום, ולא נכנס לשום חישוב.
  'home.farmExpenses.title': 'הוצאות המשק',
  'home.farmExpenses.note': 'לא נכנסות לצפי הרווח, מוצגות כאומדן בלבד',
  ...
  // שלוש היחידות, וזו רשימה סגורה. **הקוד השמור הוא באנגלית ('kg',
  // 'ton', 'unit') והמחרוזות כאן הן רק התצוגה**, כדי ששורה שנשמרה
  // היום לא תיקרא בשפה הלא נכונה ביום שתתווסף שפה שנייה. שורות ישנות
  // מחזיקות טקסט עברי חופשי, ו-yieldUnits.ts ממפה את מה שממופה ומשאיר
  // את השאר קריא כפי שנכתב.
  //
  // "קילו" ולא "ק״ג": כך היזם ועידו אמרו את זה בשיחה, וזו גם המילה
  // שעובדת אחרי ל׳ במשפט החישוב ("לקילו").
  'plots.unit.kg': 'קילו',
  'plots.unit.ton': 'טון',
  // יחיד ולא רבים, בניגוד ל"יחידות" שהיה כאן קודם: היחידה מופיעה אחרי
  // ל׳ ("ליחידה") ואחרי מספר ("24 יחידה"), והצורה ביחיד היא היחידה
  // שעובדת בשני המקומות.
  'plots.unit.unit': 'יחידה',
  ...
  // שתי יחידות ולא אחת. עידו מצהיר יבול בטון לדונם ומחיר בשקלים לקילו,
  // ועד היום שדה יחידה אחד שירת את שניהם, כלומר המספרים הוכפלו כאילו
  // הם באותה יחידה. שתי השאלות נשאלות עכשיו בנפרד וזו לצד זו.
  'plots.forecast.yieldUnitQuestion': 'באיזו יחידה היבול?',
  'plots.forecast.priceUnitQuestion': 'המחיר הוא ל...',
  // נבחר "יחידה" באחד הצדדים. ספירה לא מתורגמת למשקל, ולכן שני הצדדים
  // עוברים יחד. משפט אחד מתחת לבוררים, כדי שהקפיצה של השדה השני לא
  // תיראה כמו באג.
  'plots.forecast.unitCountLocked': 'יחידה נספרת, ולכן גם המחיר הוא ליחידה',
  // המצב שלא אמור לקרות דרך המסך (הבוררים מיישרים את שני הצדדים), אבל
  // כן אפשרי בשורה שנכתבה ישירות למסד. עדיף לומר את זה מאשר להכפיל שני
  // מספרים שלא מתאימים ולהציג תוצאה שנראית תקינה.
  'plots.forecast.unitMismatch': 'יחידת היבול ויחידת המחיר לא ניתנות להמרה זו לזו',
  'plots.forecast.unitRequired': 'צריך לבחור יחידה ליבול ויחידה למחיר',
  // מועד קטיף משוער. תאריך אחד בעתיד, מלוח שנה כמו כל תאריך אחר במוצר.
  'plots.forecast.harvestDate': 'מועד קטיף משוער',
  'plots.forecast.harvestDateClear': 'עדיין לא ידוע',
  // המשפט שמראה לחקלאי את החשבון של עצמו:
  // "8 דונם × 3 טון לדונם = 24 טון. במחיר 4 ₪ לקילו ← צפי הכנסה 96,000 ₪".
  // המילה היחידה שאין לה מקור אחר במשפט. ראה forecastSentence.
  'plots.forecast.atPrice': 'במחיר',
  ...
  // הודעת אימות אחת, לשני שדות המספר האופציונליים החדשים על הטופס
  // (יבול ומחיר), במקום שתי הודעות ספציפיות לשדה שאין ביניהן הבדל אמיתי.
  'plots.form.numberInvalid': 'המספר לא תקין, למשל 40',
  ...
  // מתחת לשם החומר במשבצת, כשעדיין אין לו מחיר במחירון. "בלי מחיר"
  // ולא "0", כי אלה שני דברים שונים בדיוק כמו ב-spray.unknownPhiDays.
  'spray.materialNoPrice': 'בלי מחיר',

  // המחירון. עידו, 6.9.2026: "לכל גידול יש את אותם חומרים חוזרים שנה
  // אחרי שנה. במקום להקליד שמות ומחירים כל פעם, תנו לי מחירון שבו אני
  // מזין את המחירים פעם אחת, ואז ביומן הריסוס שדה החומר פותח רשימה,
  // אני בוחר חומר, מקליד כמות, והכסף כבר מחושב".
  'pricelist.title': 'מחירון',
  // שורה אחת שמסבירה למה המסך קיים, וזה גם מה שהמסך הריק אומר.
  'pricelist.intro':
    'מזינים כאן מחיר לכל חומר פעם אחת, ומאז בכל ריסוס בוחרים חומר, מקלידים כמות, והעלות מחושבת לבד.',
  'pricelist.materialsTitle': 'החומרים שלכם',
  'pricelist.empty': 'המחירון עדיין ריק. הוסיפו חומר ומחיר, והם יחכו לכם בכל ריסוס.',
  'pricelist.add': 'חומר חדש',
  'pricelist.newTitle': 'חומר חדש במחירון',
  'pricelist.editTitle': 'עדכון מחיר',
  'pricelist.material': 'שם החומר',
  'pricelist.materialPlaceholder': 'לדוגמה קונפידור',
  'pricelist.unitTitle': 'לפי איזו יחידה?',
  // המחיר תמיד מוקלד ידנית ולעולם לא מנוחש: אותו חומר יכול לעלות
  // היום 7 שקלים יותר ומחר 4 פחות, והמחירון הוא הזיכרון של החקלאי
  // ולא מחירון שוק.
  'pricelist.priceHint':
    'המחיר הוא שלכם, אפשר לעדכן אותו מתי שתרצו. ריסוסים שכבר נרשמו שומרים את המחיר שהיה להם אז.',
  'pricelist.save': 'שמירת המחיר',
  'pricelist.saving': 'שומר',
  'pricelist.saved': 'המחיר נשמר',
  'pricelist.materialRequired': 'יש להזין שם חומר',
  'pricelist.priceRequired': 'יש להזין מחיר',
  'pricelist.forbidden': 'אין לכם הרשאה לערוך את המחירון',
  'pricelist.saveError': 'לא הצלחנו לשמור, נסו שוב',

  // שעות עבודה. עידו, 6.9.2026: "אני רוצה גם להזין כמה שעות לקחה
  // העבודה ומה העלות לשעת עבודה", ובנוסף: "אני לא רואה בכלל יומן שעות
  // עבודה". השעות שייכות לכל רישום ביומן ולא רק לריסוס, כי גם תיקון
  // וגם גיזום לוקחים שעות.
  'work.hours': 'שעות עבודה',
  'work.hoursPlaceholder': 'כמה שעות',
  'work.hourlyRate': 'עלות לשעה',
  'work.hourlyRatePlaceholder': 'מחיר לשעה',
  'work.cost': 'עלות העבודה',
  'work.costPlaceholder': 'סכום',
  // אותן שתי שורות כמו בעלות הריסוס, ומאותה סיבה: המספר מחושב לבד
  // כל עוד לא נגעו בו, וברגע שמקלידים סכום הוא שלכם.
  'work.costComputed': 'מחושב אוטומטית, אפשר לשנות',
  'work.costHint': 'אפשר גם פשוט להקליד את הסכום',
  'work.hoursSuffix': 'שעות',
  // סוג עבודה, החצי השני של אותה בקשה של עידו: "שעות עבודה וסוג עבודה
  // ביומן". **מחרוזת חדשה ולא שימוש חוזר ב-log.form.type**, כי אלה שתי
  // שאלות שונות על אותה שורה: 'log.form.type' הוא הרשימה הסגורה שמסך
  // יומן הריסוס והדוח לרגולטור מסננים לפיה, וזה הטקסט החופשי שאומר מה
  // באמת נעשה. שתי המחרוזות מופיעות זו מעל זו באותו גיליון, ולכן הן
  // חייבות להיקרא שונה. שני המפתחות שאחריהן מקבילים במדויק ל-
  // expense.form.addName ול-expense.form.emptyNames, אותה רשת בדיוק.
  'work.kind': 'סוג עבודה',
  'work.kindPlaceholder': 'למשל, גיזום, תיקון גדר',
  'work.kindAdd': 'סוג חדש',
  'work.kindEmpty': 'עדיין לא רשמתם סוגי עבודה. הוסיפו את הראשון והוא יופיע כאן בפעם הבאה.',

  // יומן שעות העבודה, מסך נפרד שמגיעים אליו מהיומן, בדיוק כמו יומן
  // הריסוס. עידו לא מצא את השעות בכלל, ולכן צריך מסך שאפשר להצביע
  // עליו, עם שתי הסכימות שהוא באמת שאל עליהן: כמה שעות, וכמה כסף.
  ...
  'workLog.loadError': 'לא הצלחנו לטעון את יומן השעות, נסו שוב',
  // המחירון מחזיק גם את התעריף לשעה, כי זה בדיוק "המחירים שאני מזין
  // פעם אחת". תעריף חדש לא נוגע ברישום שכבר נשמר.
  'pricelist.workTitle': 'עלות שעת עבודה',
  'pricelist.workHint':
    'התעריף הזה ממלא מראש כל רישום חדש ביומן. רישומים שכבר נשמרו שומרים את התעריף שהיה להם אז.',
  'pricelist.workRate': 'כמה עולה שעת עבודה',
  'pricelist.workSave': 'שמירת התעריף',
  'pricelist.workSaved': 'התעריף נשמר',
  'pricelist.workRateRequired': 'יש להזין מספר',
  'pricelist.workNotSet': 'עדיין לא הוזן תעריף',
