...
import { t, type CaptureKind } from '@yevul/shared';
...
// kinds מגיע מ-workerModeShell (design.md, Worker Mode): לעובד שורת
// ההוצאה מושמטת לגמרי, בדיוק כמו בגיליון הרישום, ולא מוצגת חסומה. אותה
// הפרדה: ההחלטה טהורה ב-@yevul/shared, כאן רק מרנדרים את מה שהתקבל.
export function QuickActions({ kinds }: { kinds: CaptureKind[] }) {
  ...
  return (
    <View style={styles.row}>
      {ACTIONS.filter((action) => kinds.includes(action.key)).map(
        ({ key, Icon, labelKey, tint, ink }) => (
          <Pressable
            key={key}
