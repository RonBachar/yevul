...
//
// **סרגל הייצוא נתלה על הנתונים של ExpenseList דרך renderHeader ולא
// טוען אותם בעצמו.** הגרסה הראשונה קראה כאן ל-useExpenses במקביל,
// והתוצאה הייתה שלוש שאילתות הוצאות זהות בכל טעינת מסך: אחת מתוך
// useFarmProfit, אחת כאן, ואחת מתוך ExpenseList. נמצא בקוד ריוויו
// של שלב 4.
export function MoneyScreen() {
  const { farm } = useCurrentFarm(supabase);
  const settings = useFarmSettings(supabase);
  const currency = settings.form?.currency ?? 'ILS';
  const { plots, loading: profitLoading } = useFarmProfit(supabase);
